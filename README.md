...
MAC(Multiply-Accumulate) 연산은 다음 방식으로 구현:

* 2차원 배열(패턴, 필터)의 동일 위치 값을 곱함
* 모든 곱셈 결과를 누적 합산

구현은 외부 라이브러리를 사용하지 않고 **이중 반복문**으로 직접 수행.
...
이중 for 루프 내에서 2차원 List인 pattern 및 filter들의 각 요소에 대해 mac 연산을 수행하는 구조임.
pattern과 filter의 i행 j열에 있는 요소들의 곱을 행과 열 순서로 계속 더하는 로직으로 구현하였음.
C언어로 구현된 NumPy 라이브러리를 써서 파이썬 자체의 반복문 없이 처리할수 있으니, 본 과제에서 사용불가한 영역이라서 직접 구현함.

```text
score = Σ (pattern[i][j] × filter[i][j])
```

이 방식은 모든 크기(N×N)에 대해 동일하게 동작하도록 일반화.

MAC 기반의 유사도는 벡터 형태의 패턴과 필터 간 내적(Inner Product)을 계산하는 방식으로,
값이 클수록 같은 방향을 가진다는 점을 반영하는 원리임. 세부적으로는 같은 위치에서 값이 크고, 부호가 같고, 패턴의 분포가 유사함을 나타냄
...
1. **수치 비교 문제 (부동소수점 오차)**
   ...
   부동소수점 오차는 컴퓨터는 신수를 연속적으로 저장하지 못하고 유한한 개수의 비트로 표현할 수 밖에 없어서 필연적으로 발생함.
   10진법의 무한 소수르 2진수로 정확히 표한하기 어려우며 반올림이 발생하는 구조이며 연산을 할수록 오차가 누적됨
...
즉, 시간 복잡도는 **O(N²)**이다.
단순히 반복문이 두개라서가 아니라 문제 정의 자체가 2차원 전체 공간에 대한 전수 비교를 요구함.
각 원소쌍(i,j)는 독립적으로 결과에 영향을 주기 때문에 모든 원소를 한번은 읽어서 처리해야 함.
...
### 5.1 CPU/NPU 관점의 대규모 MAC 병목 현상

* MAC 연산은 딥러닝에서 가장 주요한 역할을 하는데, 
  CPU는 범용으로 설계되어 다양한 기능을 수행하는 반면 MAC 연산과 같이 특히 단순 계산이면서 메모리 접근이 많은 경우에 병목현상이 커짐.
  즉, 병렬 처리 용량이 떨어지고, cache-DRAM-연산장치 간에 데이터 이동에 따른 병목 현상이 문제임.
  반면, NPU는 대량의 MAC 연산 처리 목적으로 설계되어 연산 능력 자체는 충분하나 일반 DRAM의 데이터 전송속도가 제한적이므로 병목현상이 생길 수 있어 HBM(High Bandwidth Memery)를 사용하여 이 부분을 해결해 가고 있음

### 5.2 1000×1000 패턴 처리 시 발생하는 문제점 및 최적화 방안

* 1000×1000 패턴 처리를 하면 O(N^3) 복잡도로 10^9 회 행렬 간 MAC 연산을 하면서 메모리 병목으로 매우 시간이 오래 걸림.
* 1. 큰 행렬을 작은 블록으로 나누어 처리(Blocking/Tilting) 2. 멀티코어/멀티스레드를 통해 병렬 처리 용량 확대
  3. 메모리 관련된 Data 이동을 줄이기 위한 알고리즘 최적화 등을 통해 개선할 수 있음
